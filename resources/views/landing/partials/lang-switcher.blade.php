@php
    /** @var array{pl_path: string, en_path: string} $landingLocaleSwitch */
    $isEn = ($landingLocale ?? 'pl') === 'en';
...
<details class="lang-switcher group/lang relative z-[70]">
    <summary
        ...
        <span class="max-w-[9rem] truncate sm:max-w-none">{{ $isEn ? __('landing.nav.lang_current_en') : __('landing.nav.lang_current_pl') }}</span>
    ...
    <div class="absolute {{ $compact ? 'left-1/2 -translate-x-1/2' : 'right-0' }} top-[calc(100%+0.45rem)] w-[min(17.5rem,calc(100vw-2rem))] origin-top overflow-hidden rounded-2xl border border-violet-100/90 bg-white/95 p-1.5 shadow-2xl shadow-violet-900/20 ring-1 ring-violet-400/15 backdrop-blur-md">
        ...
        <a href="{{ $urlPl }}" hreflang="pl" lang="pl"
            class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition {{ $isEn ? 'text-gray-800 hover:bg-gradient-to-r hover:from-violet-50 hover:to-indigo-50/50' : 'bg-gradient-to-r from-violet-100/90 to-indigo-50/90 text-violet-950 shadow-sm ring-1 ring-violet-200/70' }}">
        ...
        <a href="{{ $urlEn }}" hreflang="en" lang="en"
            class="mt-0.5 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition {{ !$isEn ? 'text-gray-800 hover:bg-gradient-to-r hover:from-violet-50 hover:to-indigo-50/50' : 'bg-gradient-to-r from-violet-100/90 to-indigo-50/90 text-violet-950 shadow-sm ring-1 ring-violet-200/70' }}">
            <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-white text-lg shadow-sm ring-1 ring-violet-100/80" aria-hidden="true">🇬🇧</span>
