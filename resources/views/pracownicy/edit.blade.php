...
@section('content')
    ...
    <form action="{{ \App\Support\AppUrl::route('kartoteki.pracownicy.update', $pracownik) }}" method="POST" style="max-width: 28rem;">
        ...
        @php
            $grupaZaznaczona = (bool) old('grupa', $pracownik->grupa);
            $wymiarWybrany = old('wymiar');
            if ($wymiarWybrany === null) {
                $wymiarWybrany = $pracownik->wymiar;
            }
        @endphp
        <div id="js-pracownik-etat-fields" style="margin-bottom: 1rem; display: {{ $grupaZaznaczona ? 'none' : 'block' }};">
            <div style="margin-bottom: 1rem;">
            ...
            <div style="margin-bottom: 1rem;">
                ...
                <div style="display: flex; align-items: center; gap: 0.5rem;">
                    <input type="number" name="wynagrodzenie" id="wynagrodzenie" step="0.01" min="0"
                           value="{{ old('wynagrodzenie', $pracownik->wynagrodzenie !== null ? $pracownik->wynagrodzenie : '') }}"
                           style="flex: 1; min-width: 0; padding: 0.5rem 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem;">
                    @if (! empty($wynagrodzenieBandFlag) && ! empty($wynagrodzenieBandTooltip))
                        @if ($wynagrodzenieBandFlag === 'below')
                            <span class="wyn-raport-band wyn-raport-band--below" title="{{ $wynagrodzenieBandTooltip }}" aria-label="{{ $wynagrodzenieBandTooltip }}">↓</span>
                        @elseif ($wynagrodzenieBandFlag === 'above')
                            <span class="wyn-raport-band wyn-raport-band--above" title="{{ $wynagrodzenieBandTooltip }}" aria-label="{{ $wynagrodzenieBandTooltip }}">↑</span>
                        @endif
                    @endif
                </div>
                @error('wynagrodzenie')
